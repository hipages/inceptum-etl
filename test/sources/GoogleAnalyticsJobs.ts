...
const gaConfig = utilConfig.get('etls.test_5.source');
...
class HelperGoogleAnalytics extends GoogleAnalyticsJobs {
  ...
  public exposeConstructDimensions() {
    return HelperGoogleAnalytics.constructDimensions(this.myDimensions['dimensions'], this.myDimensions['key']);
  }
...
suite('GoogleAnalyticsJobs', () => {
  ...
  suite('Test private methods:', () => {
    ...
    test('Test lastKeys', async () => {
      const source = new HelperGoogleAnalytics(gaConfig);
      source.getLastKey([ testObject ], myDimensions.key).must.be.equal('JOB3670766');
    });
    test('Test constructDimensions', async () => {
        const source = new HelperGoogleAnalytics(gaConfig);
        source.exposeConstructDimensions().must.be.eql(myDimensionsGroups);
    });
