...
export class GoogleAnalyticsReporting extends EtlSource {
  ...
  protected reportRequest: object;
  ...
  constructor(configGA: object) {
    ...
    this.reportRequest = configGA['reportRequest'];
    this.nextPageToken = [];
    this.rowCount = [];
    this.reportRequest['dimensions'].forEach(() => {
  ...
  protected setNextPageToken(result: Array<any>, lastKey: string) {
    ...
    result.forEach( (item, index) => {
      ...
      if ((index > 0) && (this.rowCount[index] > this.MAX_RESULTS)) {
        const rowIndex = this.gaClient.findDimensionKeyRowIndex(item, this.reportRequest['key'], lastKey, this.reportRequest['keyStringPart'].length);
        if (rowIndex !== false) {
          if (nextPageToken) {
  ...
  protected async fetchResultsFromGA() {
    const self = this;
    const dimensionToProcess: string[][] = GoogleAnalyticsReporting.constructDimensions(self.reportRequest['dimensions'], self.reportRequest['key']);
    ...
    const allPromises = dimensionToProcess.map(
      (dimension: any, index) => {
        return self.gaClient.fetch({
            ...
            metrics: self.reportRequest['metric'],
            dateRanges: self.dateRanges,
            filters: self.reportRequest['filters'],
            orderBys: self.reportRequest['key'],
    ...
    const lastKeys = self.getLastKey(result, self.reportRequest['key']);
